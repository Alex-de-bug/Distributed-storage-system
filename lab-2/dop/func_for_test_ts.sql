-- Среднесуточная температура по комнатам за последнюю неделю
CREATE OR REPLACE FUNCTION get_avg_daily_temperature_ts() RETURNS TABLE (
    date_day DATE,
...
-- Почасовое потребление электроэнергии за последний месяц
CREATE OR REPLACE FUNCTION get_hourly_electricity_consumption_ts() RETURNS TABLE (
...
BEGIN
    RETURN QUERY
    SELECT 
        time_bucket('1 hour', sr.timestamp) AS hour_timestamp,
...
-- Корреляция между температурой, влажностью и уровнем CO2
CREATE OR REPLACE FUNCTION get_environmental_correlation_ts(room_id_param INTEGER) RETURNS TABLE (
    hour_timestamp TIMESTAMPTZ,
    avg_temperature NUMERIC(10,2),
